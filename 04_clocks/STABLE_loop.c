/********
* poc version 1.1
* c main file generated from : STABLE.oc 
...
static int ISATTY;
/* Output procedures **********************/
void STABLE_O_level(void* cdata, _boolean v){
...
main(){
   /* Context allocation */
   int s = 0;
   STABLE_ctx* ctx = STABLE_new_ctx(NULL);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      printf("##### STEP %d ##################\n", ++s);
      STABLE_I_set(ctx, _get_bool("set"));
      STABLE_I_delay(ctx, _get_int("delay"));
      STABLE_step(ctx);
   }
}
